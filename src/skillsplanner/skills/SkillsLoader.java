 ...
 * subdirectory called ./Skills (from where the running program is). Call from
 * the main program in the root project directory in order for correctness.
 * Later dates may include the ability to change the directory, but that is not
 * yet implemented
 */
public class SkillsLoader {
	...
	private final URL path = FileUtils.makePath("./Skills");

	/**
	 * The constructor makes a call to getChildren in order to load the class
	 * list
	 */
	...
	/**
	 * This dynamically loads all the classes located in subdirectories
	 * (subpackages) from the current
	 */
	...
	private void loadClass(File file) throws Exception {
		...
		if (file.getName().endsWith(".class")) {
			// convert the file to a URL so that we may use the URL class
			// loaded.
			// must convert to URI first, since the file.toURL method is now
			// deprecated
			...
			String classFile = file.getPath()
					.substring(2, file.getPath().length() - 6)
					.replace("/", ".");
			classFile = classFile.trim();

			// Check for a few classes that might be loaded that aren't needed
			/**
			 * if(isException(classFile)){ return; }
			 */
	...
	/**
	 * Declare an instance of each class and return a hashmap using the class
	 * name as the lookup
	 */
	...
	/**
	 * Checks whether the class should be loaded or not based on a list of
	 * exceptions for class that don't need to be loaded
	 * 
	 * @return true if the class is an exception
	 */
	private boolean isException(String filename) {
		String[] exceptionList = { "Skills.Skills", "Skills.SkillsLoader",
